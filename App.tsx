        ...
        {activeTab === 'POS' ? (
          <div className="flex h-full overflow-hidden">
            <div className="flex-1 flex flex-col h-full overflow-hidden min-w-0">
              <div className="p-4 bg-white border-b flex gap-2 overflow-x-auto flex-nowrap hide-scrollbar items-center">
                {CATEGORIES.map(c => (
                  <button 
                    key={c} 
                    onClick={() => setActiveCategory(c)} 
                    className={`px-6 py-2 rounded-xl text-xs font-black transition-all flex-shrink-0 whitespace-nowrap ${activeCategory === c ? 'bg-orange-500 text-white shadow-lg' : 'bg-gray-100 text-gray-500'}`}
                  >
                    {c}
                  </button>
                ))}
            ...
            {/* Cart Panel - Made responsive with hidden on smaller screens logic if needed, but here improving current width behavior */}
            <div className="w-80 lg:w-96 bg-white border-l flex flex-col h-full shadow-xl flex-shrink-0">
